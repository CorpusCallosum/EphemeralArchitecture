...
Shader "Voxelform/Triplanar 3/Diffuse Normal Specular" {

    Properties {
		
		_CeilingDiffuse ("Ceiling Diffuse", 2D) = "white" {}
		_CeilingNormal ("Ceiling Normal", 2D) = "white" {}
		_CeilingSpecular ("Ceiling Specular", 2D) = "white" {}
    ...
    SubShader {
		...
		void surf (Input IN, inout CustomSurfaceOutput o)
		{
			...
			float spec2a = tex2D(_CeilingSpecular, coord2).x;
			float spec2b = tex2D(_FloorSpecular, coord2).x;
			float spec2 = (IN.worldNormal.y < 0.0) ? spec2a : spec2b;
			float spec3 = tex2D(_WallSpecular, coord3).x;

			blendedSpec = 1.0 - (spec1 * blendingWeights.x
