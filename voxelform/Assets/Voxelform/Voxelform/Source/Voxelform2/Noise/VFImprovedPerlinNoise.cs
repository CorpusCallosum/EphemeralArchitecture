/*  Author: Mark Davis
 * 
 *  Calculates a smooth volumetric noise value.
 *  The following code is based on a public sample provided by Ken Perlin.
...
namespace Voxelform2.Noise
{
	...
	/// Calculates a smooth volumetric noise value.
	/// </summary>
	public class VFImprovedPerlinNoise
	{
		...
		/// <summary>
		/// Generate smooth volumetric noise value. 
		/// </summary>
		public static float Noise(float x, float y, float z)
		{
			if (!_initialized) Initialize();

			// Find a cube that contains a point.
			int X = Mathf.FloorToInt(x) & 255;
			int Y = Mathf.FloorToInt(y) & 255;
			int Z = Mathf.FloorToInt(z) & 255;

			// Find relative x,y,z values for point in cube.
